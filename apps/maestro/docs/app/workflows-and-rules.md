...
- On a project Maestro hasn't configured yet, the canvas shows the implementation chain it
  _detected_ from the repo (which language/framework agent it thinks you need) along with the
...
- Two selectors: rules the _project_ already authors (in a flat `rules/` directory it publishes),
...
There is currently no _global_ (machine-wide) rules tier — every rule view in Maestro, including
