...
existing `kind:"handoff"` entries in `maestro_session.log.jsonl` (no new hook write; `039` adds no
...
`kind:"handoff"` entries in `maestro_session.log.jsonl`, never `kind:"dispatch"` (order-independent
with the sibling `maestro-subagent-log.js` write for the same run) and never
`maestro_session.json`. A resumed run gets one line — "Resumed run — the skills, handoff routes and
...
later run. `maestro_session.json` gained the `run_id` field this mechanism is stamped with.
...
| `SessionEnd`    | ``           | `maestro-session-cleanup.sh`      | Delete the three ephemeral session files (`maestro_session.json`, `maestro_session.log.jsonl`, `maestro_session_tasks.json`) from `<cwd>/.claude/`. **Also (`036`)**: `sweep()` `.claude/channels/` — retire `.consumed/` outright, age out anything left in a live lane past `CHANNEL_AGE_CAP_MS` (14 days), via the SAME `sweep()` the `SessionEnd` hook and the delivery hook both re-export from `lib/maestro-session.cjs`, so they cannot disagree about "too old". `maestro.json` and the orchestrator skill are deliberately preserved. No-op when the payload carries no `cwd`.                                                                                                                                                                                                                                                                                                                                                 |
...
| A handoff payload wasn't delivered, or looks stale      | check `.claude/channels/<receiver>/<sender>.1.md` — undelivered if still there, retired to `.claude/channels/.consumed/<receiver>/` if already read. Compare its stamped `run_id` (first line) against the current `maestro_session.json`; a mismatch means it will be *mentioned*, not inlined, until a matching run consumes it. |
...
- **Step 4 has the same injected-line mechanism now, for a different question (`046`).**
  ...
  when off. **`047` built the first writer**: `maestro-enable-task-routing.js`, dual-registered
...
- **A channel file's stamp, not its mtime, decides whether it's "this run" (`036`).** `writeStamp`
  ...
  stays stale forever, on purpose: `SessionEnd` deletes `maestro_session.json`, so the next run
